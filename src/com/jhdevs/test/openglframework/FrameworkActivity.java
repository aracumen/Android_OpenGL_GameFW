...
public class FrameworkActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
    	FWEngine.display = ((WindowManager) getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();

        super.onCreate(savedInstanceState);
        /*display the splash screen image*/
        setContentView(R.layout.splashscreen);
        /*start up the splash screen and main menu in a time delayed thread*/
        FWEngine.context = this;
        new Handler().postDelayed(new Thread() {
        		@Override
        		public void run() {
                   Intent mainMenu = new Intent(FrameworkActivity.this, FWMainMenu.class);
                   FrameworkActivity.this.startActivity(mainMenu);
                   FrameworkActivity.this.finish();
                   overridePendingTransition(R.layout.fadein,R.layout.fadeout);
        		}
        	}, FWEngine.GAME_THREAD_DELAY);
